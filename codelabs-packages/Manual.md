...
packages for XBPS, the `CODE LABS` native packaging system.
...
- *vsv()* `vsv <service> [<facility>]`
	...
	see [vlogger(8)](https://man.codelabs.org/vlogger.8).
...
CODE LABS uses [runit](http://smarden.org/runit/) for booting and supervision of services.
...
CODE LABS on how to write services.
...
CODE LABS's repositories goes smoothly.
...
See [`update-binfmts(8)`](https://man.codelabs.org/man8/update-binfmts.8) for more details.
...
`srcpkgs/<pkgname>/files/README.codelabs` and install with
`vdoc "${FILESDIR}/README.codelabs"`.
