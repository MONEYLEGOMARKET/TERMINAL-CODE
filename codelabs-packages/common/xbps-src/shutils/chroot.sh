...
PATH=/codelabs-packages:/usr/bin
...
chroot_handler() {
    ...
    if [ ! -d $XBPS_MASTERDIR/codelabs-packages ]; then
        mkdir -p $XBPS_MASTERDIR/codelabs-packages
    fi
    ...
    if [ "$action" = "chroot" ]; then
        ...
        env -i -- PATH="/usr/bin:$PATH" SHELL=/bin/sh \
            ...
            /codelabs-packages/xbps-src $XBPS_OPTIONS $action $pkg
        rv=$?
    fi
